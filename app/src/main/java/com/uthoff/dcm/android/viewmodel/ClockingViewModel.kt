...
class ClockingViewModel(private val user: User) : ViewModel() {
    ...
    private val _clockingTimes = MutableLiveData<List<ClockingTimes>>()
    val clockingTimes: LiveData<List<ClockingTimes>> = _clockingTimes
    ...
    private fun getLatestTimes() {
        CoroutineScope(IO).launch {
            ...
            withContext(Main) {
                if (result.isSuccessful) {
                    _clockingTimes.value = result.body()!!
                }
                else _errorMessage.value =
